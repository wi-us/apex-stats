...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--archive-before-run",
        action="store_true",
        help="Archive existing ingest DB and mp4 clips into Archieve/ before creating fresh run state.",
    )
    parser.add_argument(
        "--recent-days",
        type=int,
...
def archive_previous_run_state(db_path: Path, output_dir: Path) -> Path:
    ...
    archive_root = project_root / "Archieve"
...
def main() -> int:
    ...
    if args.archive_before_run:
        print("[ingest] archive-before-run: moving previous DB and mp4 to Archieve/")
