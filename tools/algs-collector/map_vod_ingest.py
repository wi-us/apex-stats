...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--catalog-db-path",
        default="tools/algs-collector/algs_tournaments.sqlite",
    ...
    parser.add_argument(
        "--archive-before-run",
        action="store_true",
        help="Archive existing ingest DB and mp4 clips into archive/ before creating fresh run state.",
    )
    parser.add_argument(
        "--recent-days",
        type=int,
        default=30,
        help="Process only videos published within last N days (default: 30, 0 disables filter).",
    )
    parser.add_argument(
        "--clip-20m",
        action="store_true",
        help="Force clip duration profile to 20 minutes (1200 sec).",
    )
...
def archive_previous_run_state(db_path: Path, output_dir: Path) -> Path:
    project_root = Path(__file__).resolve().parent.parent
    archive_root = project_root / "archive"
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    run_archive_dir = archive_root / f"ingest_archive_{timestamp}"
    run_archive_dir.mkdir(parents=True, exist_ok=True)

    if db_path.exists():
        archived_db_name = f"{db_path.stem}_{timestamp}{db_path.suffix or '.sqlite'}"
        archived_db_path = run_archive_dir / archived_db_name
        shutil.move(str(db_path), str(archived_db_path))
        print(f"[archive] db moved: {db_path} -> {archived_db_path}")
    else:
        print(f"[archive] db not found, skipping: {db_path}")

    moved_mp4 = 0
    if output_dir.exists():
        records_archive_dir = run_archive_dir / "records"
        for mp4_path in output_dir.rglob("*.mp4"):
            rel_path = mp4_path.relative_to(output_dir)
            target_path = records_archive_dir / rel_path
            target_path.parent.mkdir(parents=True, exist_ok=True)
            shutil.move(str(mp4_path), str(target_path))
            moved_mp4 += 1
        print(f"[archive] mp4 moved: {moved_mp4}")
    else:
        print(f"[archive] output dir not found, skipping: {output_dir}")
...
def main() -> int:
    args = parse_args()
    effective_duration_sec = 1200 if args.clip_20m else int(args.duration_sec)
    ...
    if args.archive_before_run:
        print("[ingest] archive-before-run: moving previous DB and mp4 to archive/")
        archive_dir = archive_previous_run_state(db_path=db_path, output_dir=output_dir)
        print(f"[ingest] archive dir: {archive_dir}")
    if args.hard_reset:
    ...
    print(
        f"[ingest] clip profile: {'20m (1200s forced)' if args.clip_20m else f'custom ({effective_duration_sec}s)'}"
    )
    print(f"[ingest] date filter: {'disabled' if recent_days == 0 else f'last {recent_days} day(s)'}")
    if auth_args:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        skipped_games = 0
        failed_games = 0
        filtered_by_date_videos = 0
        today_utc = datetime.now(timezone.utc).date()
        cutoff_date = (today_utc - timedelta(days=recent_days)) if recent_days > 0 else None
        ...
        for index, video_id in enumerate(video_ids, start=1):
            ...
            if cutoff_date is not None:
                ...
                if (published_date is None) or (published_date < cutoff_date):
                    filtered_by_date_videos += 1
                    print(
                        f"[video {index}/{len(video_ids)}] skip by date filter "
                        f"(published={item.published_at or 'unknown'}, cutoff={cutoff_date.isoformat()})"
                    )
                    upsert_video_mark(conn, video_id, "viewed", "filtered_by_recent_days")
                    if catalog_conn is not None:
                        catalog_conn.commit()
                    conn.commit()
                    continue
            ...
            for game_idx, game in enumerate(parsed.games, start=1):
                ...
                game_end = seconds_to_hhmmss(game.start_sec + effective_duration_sec)
                print(
        ...
        print(f"videos skipped by recent-days filter: {filtered_by_date_videos}")
