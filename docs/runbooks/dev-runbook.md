...
3. Analysis service processes map video with team settings from `services/analysis/app/core/tracking/tracking_settings.py`.
...
Archived legacy flow is stored under `archive/` (see `archive/legacy/` when present).

## Operational checklist

- Redis is up (`6379`)
- PostgreSQL is up (`5433` for local infra compose, `5432` inside server compose network)
- `FACEIT_API_KEY` is set
- API is reachable from web (`NEXT_PUBLIC_API_URL`)
- API data source flags are set to postgres:
