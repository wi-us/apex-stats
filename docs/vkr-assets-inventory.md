...
Документ фиксирует, какие схемы, скриншоты и примеры данных нужно подготовить для диплома по Apex Stats Parser. Он дополняет `docs/vkr-detailed-plan.md` и `docs/diploma-diagrams.md`.

## 1. Введение

### Схемы

- Контекстная схема системы из `docs/diploma-diagrams.md`.
- Схема ручного и автоматизированного процесса из `docs/vkr-detailed-plan.md`.
...
- Диаграмма вариантов использования из `docs/vkr-detailed-plan.md`.
...
- `videos_collector/camera_tracker.py`: трекинг камеры.
- `videos_collector/detect_map_start.py`: детекция карты/кольца/OCR.
...
Включить полный набор из `docs/diploma-diagrams.md`, при необходимости экспортировав Mermaid в PNG/SVG.
