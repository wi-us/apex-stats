...
function TeamDetail() {
  ...
  const storedTeam = teams.find((t) => t.id === teamId);
  const isLoadingStore = teams.length <= 20 && !teamId.startsWith("t-");
  const team: Team = storedTeam ?? {
    id: teamId,
    tag: "...",
    name: "Loading team",
    color: "#94a3b8",
  ...
  if (!storedTeam) {
    return (
      <div className="flex h-screen w-screen flex-col items-center justify-center gap-3 bg-background text-foreground">
        <h1 className="text-lg font-bold">{isLoadingStore ? "Loading team stats..." : "Team not found"}</h1>
        <Link to="/teams" className="text-xs text-primary hover:underline">Back to teams</Link>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="mb-4">
          <button onClick={() => navigate({ to: "/teams" })} className="text-mono rounded-sm border border-border bg-surface-2 px-2 py-1 text-xs uppercase tracking-wider hover:bg-muted">← Teams</button>
        </div>
