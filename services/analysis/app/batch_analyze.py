...
from observer_camera_compensator import (
    ObserverCameraCompensator,
    ObserverCameraConfig,
    build_observer_camera_compensator,
)
...
@dataclass
class ObserverStabilizationConfig:
    enabled: bool = True
...
def _clip_to_map_bounds(x: float, y: float, map_w: int, map_h: int) -> tuple[float, float]:
    if map_w <= 0 or map_h <= 0:
        return float(x), float(y)
    cx = min(max(0.0, float(x)), float(map_w - 1))
    cy = min(max(0.0, float(y)), float(map_h - 1))
    return cx, cy
...
def build_observer_benchmark(teams: list[TrackResult]) -> dict[str, Any]:
    total_points = 0
    valid_map_points = 0
    degraded_points = 0
    relocalized_points = 0
    frame_jitter_samples: list[float] = []
    map_jitter_samples: list[float] = []

    for team in teams:
        points = team.points or []
        prev_frame: Optional[tuple[float, float]] = None
        prev_map: Optional[tuple[float, float]] = None
        for point in points:
            total_points += 1
            state = str(point.get("transformState", "tracked"))
            if state == "degraded":
                degraded_points += 1
            if state == "relocalized":
                relocalized_points += 1

            sx = point.get("sourceFrameX", point.get("x"))
            sy = point.get("sourceFrameY", point.get("y"))
            if sx is not None and sy is not None:
                curr_frame = (float(sx), float(sy))
                if prev_frame is not None:
                    frame_jitter_samples.append(_distance(prev_frame, curr_frame))
                prev_frame = curr_frame

            map_x = point.get("mapX")
            map_y = point.get("mapY")
            if map_x is not None and map_y is not None:
                curr_map = (float(map_x), float(map_y))
                valid_map_points += 1
                if prev_map is not None:
                    map_jitter_samples.append(_distance(prev_map, curr_map))
                prev_map = curr_map
    ...
    return {
        ...
        "validMapPointRatio": float(valid_map_points / max(1, total_points)),
        "degradedPointRatio": float(degraded_points / max(1, total_points)),
        "relocalizedPointRatio": float(relocalized_points / max(1, total_points)),
        "legacyFrameSpaceJitterPxAvg": _safe_mean(frame_jitter_samples),
        "stabilizedMapSpaceJitterPxAvg": _safe_mean(map_jitter_samples),
        "note": "legacyFrameSpaceJitterPxAvg approximates baseline pipeline; stabilizedMapSpaceJitterPxAvg uses map-space projection.",
    }
...
def _resolve_env_for_pg_sync() -> dict[str, str]:
    env = dict(os.environ)
    env_from_file = _read_simple_dotenv(PROJECT_ROOT / ".env")
...
def _should_sync_postgres(disable_flag: bool) -> bool:
    if disable_flag:
        return False
    env = _resolve_env_for_pg_sync()
    source = str(env.get("CATALOG_SOURCE", "")).strip().lower()
    if source not in ("postgres", "hybrid"):
        return False
    if str(env.get("DATABASE_URL", "")).strip():
        return True
    host = str(env.get("POSTGRES_HOST", "")).strip()
    user = str(env.get("POSTGRES_USER", "")).strip()
    db = str(env.get("POSTGRES_DB", "")).strip()
    return bool(host and user and db)


def sync_output_to_postgres(output_json_path: Path) -> tuple[bool, str]:
    script_path = PROJECT_ROOT / "Server" / "scripts" / "upsert_analysis_result_to_pg.js"
    if not script_path.exists():
        return False, f"sync script not found: {script_path}"
    try:
        proc = subprocess.run(
            ["node", str(script_path), "--file", str(output_json_path)],
        ...
        return False, f"failed to run sync script: {exc}"
...
def infer_match_map_ids_from_video_name(video_name: str) -> tuple[Optional[str], Optional[str]]:
    name = str(video_name or "").strip()
    if not name:
        return None, None
    base = Path(name).name
    pattern = re.compile(r"_G(?P<game>\d+)_VID_(?P<video>[A-Za-z0-9_-]+)", re.IGNORECASE)
    match = pattern.search(base)
    if not match:
        return None, None
    game_number = int(match.group("game"))
    video_id = str(match.group("video"))
    if game_number <= 0 or not video_id:
        return None, None
    match_id = f"match_{video_id}"
    map_id = f"{match_id}_r{game_number}"
    return match_id, map_id


def infer_map_number_from_video_name(video_name: str) -> Optional[int]:
    name = str(video_name or "").strip()
    if not name:
        return None
    match = re.search(r"_G(?P<game>\d+)_VID_", Path(name).name, re.IGNORECASE)
    if not match:
        return None
    game_number = int(match.group("game"))
    return game_number if game_number > 0 else None
...
def load_round_windows_from_rings(
    ...
    try:
        ...
        if video_duration_sec > round1_start:
            all_end = float(video_duration_sec)
        else:
...
def load_zone_rules(zones_file: Optional[str], map_name: str) -> Optional[dict[str, Any]]:
    """Load zone polygons for map-level filtering and respawn-priority search."""
    normalized_map = normalize_map_name(map_name)
    ...
    logger.info(
        "Zone filter loaded: %s (forbidden polygons: %d; respawn disabled)",
        resolved_path,
...
def build_respawn_search_rois(
    zone_rules: Optional[dict[str, Any]],
) -> list[tuple[int, int, int, int]]:
    # Respawn-priority search is intentionally disabled.
    return []
...
def analyze_team_task(
    ...
    (
        ...
        frame_skip,
        elimination_info,
        observer_cfg_payload,
        observer_camera_payload,
        run_command,
        run_started_wall,
        status_interval_sec,
        progress_queue,
    ...
    return analyze_team(
        ...
        observer_cfg=ObserverStabilizationConfig(**observer_cfg_payload),
        observer_camera_payload=observer_camera_payload,
...
def render_debug_windows(
    ...
    if show_polygons_map and zone_preview:
        ...
        cv2.putText(preview, "respawn", (290, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors["respawn"], 1)
...
def analyze_team(
    ...
    observer_cfg: Optional[ObserverStabilizationConfig] = None,
    observer_camera_payload: Optional[dict[str, Any]] = None,
    progress_callback: Optional[Callable[[int, int, float], None]] = None,
    zone_preview: Optional[dict[str, Any]] = None,
    show_polygons_map: bool = False,
    progress_interval_sec: float = 1.0,
    ...
    observer_cfg = observer_cfg or ObserverStabilizationConfig()
    observer_compensator = ObserverCameraCompensator.from_payload(observer_camera_payload) if observer_cfg.enabled else None
    ...
    detection_min_area = team_config.get("min_area", config.MIN_ARROW_AREA)
    detection_max_area = team_config.get("max_area", config.MAX_ARROW_AREA)
    ...
    map_x, map_y, map_w, map_h = config.MAP_ROI
    registration_engine: Optional[MapRegistrationEngine] = None
    bg_estimator: Optional[BackgroundConfidenceEstimator] = None
    transform_state = "degraded"
    transform_failures = 0
    transform_residual = 999.0
    bg_confidence = 0.0
    feature_inlier_score = 0.0
    transform_delta_px = 0.0
    last_observation_frame = -10**9
    point_quality_snapshots: list[dict[str, Any]] = []
    map_dims = (map_w, map_h)
    current_transform = SimilarityTransform(
        scale=1.0,
        rotation_rad=0.0,
        tx=float(-map_x),
        ty=float(-map_y),
    ...
    if observer_compensator is not None:
        map_dims = (observer_compensator.map_width, observer_compensator.map_height)
        transform_state = "tracked"
        bg_confidence = 1.0
        transform_residual = 0.0
        feature_inlier_score = 1.0
    elif observer_cfg.enabled:
        ref_path = resolve_reference_map_path(PROJECT_ROOT, map_name, observer_cfg.map_image_path)
        reference_map = load_image_safe(ref_path) if ref_path is not None else None
        if reference_map is None:
            logger.warning("[%s/%s] observer stabilization disabled: reference map image not found", team_id, team_name)
        else:
            registration_engine = MapRegistrationEngine(
                reference_map=reference_map,
                map_roi=config.MAP_ROI,
                config=RegistrationConfig(smoothing_alpha=float(observer_cfg.smoothing_alpha)),
            )
            bg_estimator = BackgroundConfidenceEstimator(
                low_threshold=float(observer_cfg.low_threshold),
                high_threshold=float(observer_cfg.high_threshold),
                check_every_frames=int(observer_cfg.check_every_frames),
            )
            current_transform = registration_engine.current_transform
            transform_state = "tracked"
            map_dims = (registration_engine.reference_w, registration_engine.reference_h)
    ...
    while True:
        ...
        if after_traj_len > before_traj_len:
            ...
            tracking_pos = getattr(tracker, "tracking_position", None)
            if isinstance(tracking_pos, tuple) and len(tracking_pos) == 2:
                source_x = float(tracking_pos[0])
                source_y = float(tracking_pos[1])
            else:
                source_x = float(map_x + map_w / 2.0)
                source_y = float(map_y + map_h / 2.0)
            map_x_proj: Optional[float] = None
            map_y_proj: Optional[float] = None
            observer_event: Optional[str] = None
            observer_scale = float(current_transform.scale)
            observer_transform_scale = float(current_transform.scale)
            map_space_valid = (transform_state != "degraded")
            if observer_compensator is not None:
                projection = observer_compensator.project_point(timestamp, source_x, source_y)
                map_x_proj = projection.x
                map_y_proj = projection.y
                map_space_valid = bool(projection.map_space_valid)
                bg_confidence = float(projection.confidence)
                transform_residual = float(projection.residual_px)
                transform_state = str(projection.state)
                transform_delta_px = float(projection.delta_px)
                feature_inlier_score = float(projection.feature_inlier_score)
                observer_event = projection.event_type
                observer_scale = float(projection.cumulative_zoom_ratio)
                observer_transform_scale = float(projection.scale)
            elif map_space_valid:
                mx, my = (
                    registration_engine.project_frame_point(source_x, source_y, current_transform)
                    if registration_engine is not None
                    else (source_x + float(current_transform.tx), source_y + float(current_transform.ty))
                )
                map_x_proj, map_y_proj = _clip_to_map_bounds(mx, my, map_dims[0], map_dims[1])
            point_snapshot = {
                "mapX": map_x_proj,
                "mapY": map_y_proj,
                "bgConfidence": float(bg_confidence),
                "transformResidual": float(transform_residual),
                "transformState": str(transform_state),
                "transformDeltaPx": float(transform_delta_px),
                "featureInlierScore": float(feature_inlier_score),
                "sourceFrameX": float(source_x),
                "sourceFrameY": float(source_y),
                "mapSpaceValid": bool(map_space_valid),
                "backupFrameSpace": bool(not map_space_valid),
                "observerEvent": observer_event,
                "observerDeltaPx": float(transform_delta_px),
                "observerScale": float(observer_scale),
                "observerTransformScale": float(observer_transform_scale),
            }
            for _ in range(after_traj_len - before_traj_len):
                point_quality_snapshots.append(dict(point_snapshot))
        ...
        map_roi_image = frame[map_y : map_y + map_h, map_x : map_x + map_w]
        if (
            registration_engine is not None
            and bg_estimator is not None
            and map_roi_image.size > 0
            and (frame_num - last_observation_frame) >= int(bg_estimator.check_every_frames)
        ):
            last_observation_frame = frame_num
            observation = registration_engine.estimate_from_roi(map_roi_image, (map_x, map_y))
            if observation is not None:
                transform_delta_px = registration_engine.transform_delta_px(
                    current_transform,
                    observation.transform,
                    sample_point=(float(map_x + map_w / 2.0), float(map_y + map_h / 2.0)),
                )
                bg_parts = bg_estimator.evaluate(
                    map_roi_image,
                    feature_inlier_score=observation.inlier_ratio,
                    transform_delta_px=transform_delta_px,
                    residual_px=observation.residual_px,
                )
                feature_inlier_score = float(bg_parts.feature_inlier_score)
                bg_confidence = float(bg_parts.fused_score)
                transform_residual = float(observation.residual_px)
                if bg_estimator.needs_relocalization(
                    bg_confidence,
                    currently_degraded=(transform_state == "degraded"),
                ):
                    registration_engine.current_transform = observation.transform
                    current_transform = observation.transform
                    transform_state = "relocalized"
                else:
                    current_transform = registration_engine.update_transform(observation)
                    transform_state = "tracked"
                transform_failures = 0
            else:
                feature_inlier_score = 0.0
                transform_residual = 999.0
                bg_confidence = max(0.0, bg_confidence * 0.85)
                transform_failures += 1
                if transform_failures >= int(observer_cfg.fail_limit):
                    transform_state = "degraded"
        elif registration_engine is None:
            transform_state = "tracked"
            bg_confidence = 1.0
            transform_residual = 0.0
            feature_inlier_score = 1.0
    ...
    if len(point_quality_snapshots) < len(points):
        missing = len(points) - len(point_quality_snapshots)
        for idx in range(missing):
            source_x = float(points[len(point_quality_snapshots) + idx].get("x", map_x + map_w / 2.0))
            source_y = float(points[len(point_quality_snapshots) + idx].get("y", map_y + map_h / 2.0))
            point_timestamp = float(points[len(point_quality_snapshots) + idx].get("timestamp", final_timestamp))
            map_x_proj: Optional[float] = None
            map_y_proj: Optional[float] = None
            observer_event: Optional[str] = None
            observer_scale = float(current_transform.scale)
            ...
            map_space_valid = (transform_state != "degraded")
            if observer_compensator is not None:
                projection = observer_compensator.project_point(point_timestamp, source_x, source_y)
                map_x_proj = projection.x
                map_y_proj = projection.y
                map_space_valid = bool(projection.map_space_valid)
                bg_confidence = float(projection.confidence)
                ...
                map_x_proj, map_y_proj = _clip_to_map_bounds(mx, my, map_dims[0], map_dims[1])
            point_quality_snapshots.append(
                {
                    "mapX": map_x_proj,
                    "mapY": map_y_proj,
                    "bgConfidence": float(bg_confidence),
                    ...
                    "transformDeltaPx": float(transform_delta_px),
                    "featureInlierScore": float(feature_inlier_score),
                    "sourceFrameX": source_x,
                    "sourceFrameY": source_y,
                    "mapSpaceValid": bool(map_space_valid),
                    "backupFrameSpace": bool(not map_space_valid),
                    "observerEvent": observer_event,
                    "observerDeltaPx": float(transform_delta_px),
    ...
    if len(point_quality_snapshots) > len(points):
        point_quality_snapshots = point_quality_snapshots[: len(points)]
    for idx, point in enumerate(points):
        quality = point_quality_snapshots[idx] if idx < len(point_quality_snapshots) else {}
        point["sourceFrameX"] = float(quality.get("sourceFrameX", point.get("x", 0.0)))
        point["sourceFrameY"] = float(quality.get("sourceFrameY", point.get("y", 0.0)))
        point["mapX"] = quality.get("mapX")
        point["mapY"] = quality.get("mapY")
        point["bgConfidence"] = float(quality.get("bgConfidence", 0.0))
        point["transformResidual"] = float(quality.get("transformResidual", 999.0))
        point["transformState"] = str(quality.get("transformState", "degraded"))
        point["transformDeltaPx"] = float(quality.get("transformDeltaPx", 0.0))
        point["featureInlierScore"] = float(quality.get("featureInlierScore", 0.0))
        point["mapSpaceValid"] = bool(quality.get("mapSpaceValid", False))
        point["backupFrameSpace"] = bool(quality.get("backupFrameSpace", True))
        point["observerEvent"] = quality.get("observerEvent")
        point["observerDeltaPx"] = float(quality.get("observerDeltaPx", quality.get("transformDeltaPx", 0.0)))
        point["observerScale"] = float(quality.get("observerScale", 1.0))
        point["observerTransformScale"] = float(quality.get("observerTransformScale", quality.get("observerScale", 1.0)))
    ...
    diagnostics = {
        ...
        "observerStabilizationEnabled": bool(observer_cfg.enabled),
        "observerTimelineEnabled": bool(observer_compensator is not None),
        "observerTimelineSamples": int(len(observer_compensator.samples) if observer_compensator is not None else 0),
        "observerTimelineEvents": int(len(observer_compensator.events) if observer_compensator is not None else 0),
        "mapSpaceValidPoints": int(sum(1 for point in points if bool(point.get("mapSpaceValid")))),
        "registrationState": str(transform_state),
        "registrationBgConfidence": float(bg_confidence),
        "registrationResidual": float(transform_residual),
        "registrationFailCount": int(transform_failures),
    }
...
def main():
    ...
    parser.add_argument(
        "--disable-observer-stabilization",
        action="store_true",
        help="Disable dynamic map registration and keep legacy frame-space behavior.",
    )
    parser.add_argument(
        "--stabilization-check-every",
        type=int,
        default=10,
        help="Fine observer registration step in frames after a coarse pan/zoom candidate is found.",
    )
    parser.add_argument(
        "--observer-coarse-check-every",
        type=int,
        default=120,
        help="Coarse observer scan step in frames before rolling back to the fine step around pan/zoom candidates.",
    )
    parser.add_argument(
        "--observer-refine-window-sec",
        type=float,
        default=2.5,
        help="Extra seconds around a coarse pan/zoom candidate to rescan with the fine observer step.",
    )
    parser.add_argument(
        "--stabilization-low-threshold",
        type=float,
        default=0.45,
        help="Low hysteresis threshold for background confidence.",
    )
    parser.add_argument(
        "--stabilization-high-threshold",
        type=float,
        default=0.65,
        help="High hysteresis threshold for background confidence recover.",
    )
    parser.add_argument(
        "--stabilization-fail-limit",
        type=int,
        default=3,
        help="Consecutive failed registration checks before degraded state.",
    )
    parser.add_argument(
        "--stabilization-smoothing-alpha",
        type=float,
        default=1.0,
        help="Smoothing factor for temporal transform update.",
    )
    parser.add_argument(
        "--stabilization-map-image",
        default="",
        help="Optional explicit map image for registration.",
    )
    parser.add_argument(
        "--stabilization-hold-last-good-sec",
        type=float,
        default=1.25,
        help="How long to reuse the last good observer transform during short registration gaps.",
    )
    parser.add_argument(
        "--observer-event-delta-px",
        type=float,
        default=18.0,
        help="Pan jump threshold for observer camera event detection.",
    )
    parser.add_argument(
        "--observer-event-zoom-ratio",
        type=float,
        default=0.025,
        help="Relative scale jump threshold for observer zoom event detection.",
    )
    parser.add_argument(
        "--observer-event-min-inlier-ratio",
        type=float,
        default=0.25,
        help="Minimum feature inlier ratio for accepting large observer pan/zoom jumps as relocalized events.",
    )
    parser.add_argument(
        "--observer-event-max-residual-px",
        type=float,
        default=8.0,
        help="Maximum registration residual for accepting large observer pan/zoom jumps as relocalized events.",
    )
    parser.add_argument(
        "--benchmark-pan-zoom",
        action="store_true",
        help="Write observer pan/zoom benchmark section to output JSON.",
    )
    parser.add_argument(
        "--disable-postgres-sync",
        action="store_true",
        help="Do not auto-sync fresh analysis output to PostgreSQL.",
    )
    args = parser.parse_args()
    observer_cfg = ObserverStabilizationConfig(
        enabled=not bool(args.disable_observer_stabilization),
        check_every_frames=max(1, int(args.stabilization_check_every)),
        coarse_check_every_frames=max(1, int(args.observer_coarse_check_every)),
        refine_check_every_frames=max(1, int(args.stabilization_check_every)),
        refine_window_sec=max(0.0, float(args.observer_refine_window_sec)),
        low_threshold=float(args.stabilization_low_threshold),
        high_threshold=float(args.stabilization_high_threshold),
        fail_limit=max(1, int(args.stabilization_fail_limit)),
        smoothing_alpha=max(0.0, min(1.0, float(args.stabilization_smoothing_alpha))),
        map_image_path=(args.stabilization_map_image.strip() or None),
        hold_last_good_sec=max(0.0, float(args.stabilization_hold_last_good_sec)),
        event_delta_px=max(0.0, float(args.observer_event_delta_px)),
        event_zoom_ratio=max(0.0, float(args.observer_event_zoom_ratio)),
        min_event_inlier_ratio=max(0.0, min(1.0, float(args.observer_event_min_inlier_ratio))),
        max_event_residual_px=max(0.0, float(args.observer_event_max_residual_px)),
    )
    ...
    map_start_record = load_map_start_record(map_start_db_path, video_path.name)
    user_provided_map = "--map" in sys.argv

    normalized_map_name = normalize_map_name(args.map)
    map_start_seconds: Optional[float] = None
    if map_start_record and (args.use_map_start_db or not user_provided_map):
    ...
    elif args.use_map_start_db:
        logger.warning("[map-start-db] no record for video: %s", video_path.name)

    inferred_match_id, inferred_map_id = infer_match_map_ids_from_video_name(video_path.name)
    inferred_map_number = infer_map_number_from_video_name(video_path.name)

    match_id = slugify_for_filename(
        args.match_id
        if (args.match_id and args.match_id != "test")
        else (inferred_match_id or "test")
    )
    tournament_id = slugify_for_filename(args.tournament_id or "test")
    map_number = resolve_map_number(match_id, args.map_number or inferred_map_number)
    if args.map_id and args.map_id.strip():
        effective_map_id = args.map_id.strip()
    elif inferred_map_id:
        effective_map_id = inferred_map_id
    else:
        effective_map_id = f"{match_id}_map{map_number}"
    output_path = build_output_path(match_id, map_number, normalized_map_name, args.output)
    if args.team is not None and not args.output:
        # Team-specific debug runs should not overwrite primary all-teams artifact.
        output_path = output_path.with_name(f"{output_path.stem}_team_{int(args.team)}{output_path.suffix}")
    ...
    persist_job("running")
    observer_cfg_payload = asdict(observer_cfg)
    observer_camera_payload: Optional[dict[str, Any]] = None
    observer_camera_summary: dict[str, Any] = {"enabled": bool(observer_cfg.enabled), "samples": 0, "events": 0}
    if observer_cfg.enabled:
        current_action = "building observer camera timeline"
        emit_progress_line("ALL", 0.0, "observer", float(start_seconds))
        persist_job("running", force=True)
        observer_camera = build_observer_camera_compensator(
            ...
            config=ObserverCameraConfig(**observer_cfg_payload),
        )
        observer_camera_payload = observer_camera.to_payload()
        observer_camera_summary = dict(observer_camera_payload.get("summary", observer_camera.summary()))
        logger.info("Observer camera timeline ready: %s", observer_camera_summary)
        emit_progress_line(
            "ALL",
            0.0,
            f"observer_samples={observer_camera_summary.get('samples', 0)}",
            float(start_seconds),
        )
    ...
    if args.visualize or args.workers <= 1 or len(team_items) == 1:
        ...
        enable_worker_progress_queue = os.name != "nt"
        progress_queue_mp = MPQueue(maxsize=8192) if enable_worker_progress_queue else None
        tasks = [
            (
                video_path_str,
                fps,
                ...
                started_wall,
                status_interval_sec,
                progress_queue_mp if enable_worker_progress_queue else None,
            )
            for team_id, team_config in team_items
        ]
        ...
        progress_drainer: Optional[threading.Thread] = None
        if progress_queue_mp is not None:
    ...
    observer_benchmark = build_observer_benchmark(results) if args.benchmark_pan_zoom else None
        ...
        json.dump(
            {
                ...
                "observerStabilization": asdict(observer_cfg),
                "observerCamera": observer_camera_summary,
                "observerBenchmark": observer_benchmark,
            },
            file,
            ensure_ascii=False,
            indent=2
    ...
    if _should_sync_postgres(bool(args.disable_postgres_sync)):
        if not results:
            logger.warning("Postgres auto-sync skipped: no team results in output payload.")
        elif args.team is not None:
            logger.warning("Postgres auto-sync skipped for --team debug run to avoid wiping full-map tracks.")
        else:
            ok, detail = sync_output_to_postgres(output_path)
            if ok:
                logger.info("Postgres auto-sync done: %s", detail)
            else:
                logger.warning("Postgres auto-sync failed: %s", detail)
