...
export class CatalogService {
  ...
  private readonly maps: MapEntry[] = [
    {
      ...
      ring1StartSec: 180,
      ring2StartSec: 420,
      observerRoi: { ...DEFAULT_OBSERVER_ROI },
    }
  ];
  ...
  private async getTracksFromPostgres(mapId: string, teamIds?: string[], fromSec?: number, toSec?: number): Promise<TeamTrack[]> {
    ...
    let rows: Array<{
      ...
      map_x?: number | null;
      map_y?: number | null;
      source_frame_x?: number | null;
      source_frame_y?: number | null;
      map_space_valid?: boolean | null;
      backup_frame_space?: boolean | null;
      transform_state?: string | null;
      transform_residual?: number | null;
      bg_confidence?: number | null;
    ...
    try {
      rows = await this.pgQuery<{
        ...
        `SELECT team_id, timestamp_sec, x, y, confidence, map_x, map_y, source_frame_x, source_frame_y, map_space_valid, backup_frame_space, transform_state, transform_residual, bg_confidence, eliminated, elimination_timestamp_sec, elimination_frame, elimination_confidence, elimination_method
         FROM team_tracks
      ...
      // Backward compatibility for DBs without extended map-space columns yet.
      const pgErr = error as { code?: string };
      if (pgErr?.code !== "42703") throw error;
      rows = await this.pgQuery<{
        team_id: string;
        timestamp_sec: number;
        x: number;
    ...
    for (const row of rows) {
      ...
      byTeam.get(row.team_id)!.points.push({
        ...
        mapX: row.map_x !== undefined && row.map_x !== null ? Number(row.map_x) : null,
        mapY: row.map_y !== undefined && row.map_y !== null ? Number(row.map_y) : null,
        sourceFrameX: row.source_frame_x !== undefined && row.source_frame_x !== null ? Number(row.source_frame_x) : undefined,
        sourceFrameY: row.source_frame_y !== undefined && row.source_frame_y !== null ? Number(row.source_frame_y) : undefined,
        mapSpaceValid: row.map_space_valid !== undefined && row.map_space_valid !== null ? Boolean(row.map_space_valid) : undefined,
        backupFrameSpace:
          row.backup_frame_space !== undefined && row.backup_frame_space !== null ? Boolean(row.backup_frame_space) : undefined,
        transformState: row.transform_state ?? undefined,
        transformResidual:
          row.transform_residual !== undefined && row.transform_residual !== null ? Number(row.transform_residual) : undefined,
        bgConfidence: row.bg_confidence !== undefined && row.bg_confidence !== null ? Number(row.bg_confidence) : undefined,
      });
    }
  ...
  private async getRingsFromPostgres(mapId: string, fromSec?: number, toSec?: number): Promise<RingPoint[]> {
    ...
    const mapped: RingPoint[] = rows
      .map((row) => ({
        ...
        ringNumber: Number(row.segment),
        confidence: Number(row.confidence ?? 1),
        coordinateSpace: "frame" as const,
      }))
      .sort((a, b) => a.timestampSec - b.timestampSec);
    for (let i = 0; i < mapped.length; i++) {
      const current = mapped[i];
      const next = mapped[i + 1];
      current.timeStartSec = current.timestampSec;
      current.timeEndSec = next ? next.timestampSec : Number.POSITIVE_INFINITY;
    }
    return mapped.filter((row) => {
      const start = Number(row.timeStartSec ?? row.timestampSec);
      const end = Number(row.timeEndSec ?? row.timestampSec);
      if (fromSec !== undefined && end < fromSec) return false;
      if (toSec !== undefined && start > toSec) return false;
      return true;
    });
  }
  ...
  async getTournaments() {
    if (this.isPostgresEnabled()) {
      try {
        ...
        if (this.catalogSourceMode === "postgres") return pgRows;
        if (pgRows.length) return pgRows;
      } catch (error) {
        if (this.catalogSourceMode === "postgres") throw error;
  ...
  async getTeamsForMap(mapId: string): Promise<Team[]> {
    if (this.isPostgresEnabled()) {
      try {
        ...
        if (this.catalogSourceMode === "postgres") return pgTeams;
  ...
  async getTracks(mapId: string, teamIds?: string[], fromSec?: number, toSec?: number) {
    if (this.isPostgresEnabled()) {
      try {
        ...
        if (this.catalogSourceMode === "postgres") return pgTracks;
    ...
    const picked = record.teams
      .map((team) => ({
        ...
        points: (Array.isArray(team.points) ? team.points : []).map((point) => ({
          ...
          mapX: point.mapX === null || point.mapX === undefined ? null : Number(point.mapX),
          mapY: point.mapY === null || point.mapY === undefined ? null : Number(point.mapY),
          sourceFrameX: point.sourceFrameX !== undefined ? Number(point.sourceFrameX) : undefined,
          sourceFrameY: point.sourceFrameY !== undefined ? Number(point.sourceFrameY) : undefined,
          mapSpaceValid: point.mapSpaceValid !== undefined ? Boolean(point.mapSpaceValid) : undefined,
          backupFrameSpace: point.backupFrameSpace !== undefined ? Boolean(point.backupFrameSpace) : undefined,
          transformState: point.transformState !== undefined ? String(point.transformState) : undefined,
          transformResidual: point.transformResidual !== undefined ? Number(point.transformResidual) : undefined,
          bgConfidence: point.bgConfidence !== undefined ? Number(point.bgConfidence) : undefined,
        })),
  ...
  /**
   * `center` JSON from map-start pipeline always includes `countdown_ring_number` even for CLOSING rows
   * (it denotes the *next* ring countdown). Do not infer phase from presence of that field alone.
   */
  private resolveSqliteRingEventType(
    rowRingNumber: number,
    center: {
      ring_event_type?: string;
      countdown_ring_number?: number;
      ring_number_source?: string;
    }
  ): "closing" | "countdown" {
    const explicit = String(center.ring_event_type ?? "").toLowerCase();
    if (explicit === "countdown") return "countdown";
    if (explicit === "closing") return "closing";
    const src = String(center.ring_number_source ?? "").toLowerCase();
    if (src === "countdown_minus_one" || src === "backfill_r2_countdown") {
      return "closing";
    }
    const cr = center.countdown_ring_number != null ? Number(center.countdown_ring_number) : Number.NaN;
    if (Number.isFinite(cr) && Number.isFinite(rowRingNumber) && cr === rowRingNumber + 1) {
      return "closing";
    }
    return Number.isFinite(cr) && cr > 0 ? "countdown" : "closing";
  }
      ...
      this.withSqlite(
        ...
        (db) => {
          ...
          for (const row of rows) {
            ...
            try {
              center = JSON.parse(row.center) as { x?: number; y?: number; space?: string };
            } catch {
              center = {};
            }
            ...
            const coordinateSpace = String(center.space ?? "").toLowerCase() === "map" ? "map" : "frame";
            const sqlRingNo = Number(row.ring_number ?? 1);
            points.push({
              mapId,
              timestampSec: Number(row.time_start ?? 0),
              timeStartSec: Number(row.time_start ?? 0),
              timeEndSec: row.time_end !== null ? Number(row.time_end) : Number.POSITIVE_INFINITY,
              ringNumber: sqlRingNo,
              ringEventType: this.resolveSqliteRingEventType(sqlRingNo, center),
              countdownRingNumber:
                center.countdown_ring_number !== undefined && center.countdown_ring_number !== null
                  ? Number(center.countdown_ring_number)
                  : undefined,
              ringNumberSource: center.ring_number_source ? String(center.ring_number_source) : undefined,
              timingSource: center.timing_source ? String(center.timing_source) : undefined,
              x,
              y,
        ...
        const start = Number(ring.timeStartSec ?? ring.timestampSec);
        const end = Number(ring.timeEndSec ?? ring.timestampSec);
        if (fromSec !== undefined && end < fromSec) return false;
        if (toSec !== undefined && start > toSec) return false;
        return true;
      });

    const sqliteRings = fromSqlite();
    if (sqliteRings.length) {
      return sqliteRings;
    }
    if (this.isPostgresEnabled()) {
      try {
        const pgRings = await this.getRingsFromPostgres(mapId, fromSec, toSec);
        if (this.catalogSourceMode === "postgres") return pgRings;
  ...
  async getCameraTracks(mapId: string, fromSec?: number, toSec?: number): Promise<CameraTrackPoint[]> {
    return this.withSqlite(
      this.cameraDbPath,
      (db) => {
        const gameId = this.mapIdToGameId(mapId);
        if (!gameId) return [] as CameraTrackPoint[];
        const rows = db
          .prepare(
            `
            SELECT timestamp, ring_status, ring_number, center_x, center_y, camera_x, camera_y, radius,
                   zoom_ratio, camera_size, roi_x1, roi_y1, roi_x2, roi_y2, jump_score, jump_flag,
                   x1, x2, y1, y2, move_dx, move_dy, move_dist, move_side
            FROM CameraTrack
            WHERE game_id = ?
          ...
          .all(gameId) as Array<{
            ...
            move_dx: number | null;
            move_dy: number | null;
            move_dist: number | null;
            move_side: string | null;
          }>;
        const points = rows
          .map((row) => ({
            mapId,
            timestampSec: Number(row.timestamp ?? 0),
            ringStatus: (String(row.ring_status ?? "countdown") === "closing" ? "closing" : "countdown") as
              | "closing"
              | "countdown",
            ringNumber: Number(row.ring_number ?? 0),
            centerX: Number(row.center_x ?? 0),
            centerY: Number(row.center_y ?? 0),
            cameraX: Number(row.camera_x ?? 540),
            cameraY: Number(row.camera_y ?? 540),
            radius: Number(row.radius ?? 0),
            zoomRatio: Number(row.zoom_ratio ?? 1),
            cameraSize: Number(row.camera_size ?? 1080),
            roiX1: Number(row.roi_x1 ?? 0),
            roiY1: Number(row.roi_y1 ?? 0),
            roiX2: Number(row.roi_x2 ?? 1079),
            roiY2: Number(row.roi_y2 ?? 1079),
            jumpScore: Number(row.jump_score ?? 0),
            jumpFlag: Boolean(Number(row.jump_flag ?? 0)),
            x1: Number(row.x1 ?? 0),
            x2: Number(row.x2 ?? 0),
            y1: Number(row.y1 ?? 0),
            y2: Number(row.y2 ?? 0),
            moveDx: Number(row.move_dx ?? 0),
            moveDy: Number(row.move_dy ?? 0),
            moveDist: Number(row.move_dist ?? 0),
            moveSide: String(row.move_side ?? "none"),
          }))
          .filter((row) => {
            if (fromSec !== undefined && row.timestampSec < fromSec) return false;
  ...
  getMapBackgroundPath(mapId: string): string | null {
    ...
    const mapName =
      record?.mapName ??
      this.mapIdToDetectedMapName(mapId) ??
      this.maps.find((map) => map.id === mapId)?.mapName ??
      mapId;
    if (!mapName) {
      return null;
    }
  ...
  getMapVideoPath(mapId: string): string | null {
    ...
    const record = this.findAnalysisRecordByMapId(mapId);
    pushCandidate(record?.video);
    const mapEntry = this.maps.find((map) => map.id === mapId);
    pushCandidate(mapEntry?.videoUrl);
  ...
  private resolveExistingZonesPath(preferredPath: string, mapName: string): string {
    if (fs.existsSync(preferredPath)) return preferredPath;
    const zonesDir = this.runtimePaths.artifacts.zonesDir;
    if (!fs.existsSync(zonesDir)) return preferredPath;
    const wantedKey = this.normalizeMapAssetKey(mapName);
    const files = fs.readdirSync(zonesDir).filter((name) => name.toLowerCase().endsWith(".zones.json"));
    for (const fileName of files) {
      const mapNameFromFile = fileName.replace(/\.zones\.json$/i, "");
      if (this.normalizeMapAssetKey(mapNameFromFile) === wantedKey) {
        return path.join(zonesDir, fileName);
      }
    }
  ...
  private resolveExistingTextZonesPath(preferredPath: string, mapName: string): string {
    if (fs.existsSync(preferredPath)) {
      return preferredPath;
    }
    const textZonesDir = this.runtimePaths.artifacts.textZonesDir;
    if (!fs.existsSync(textZonesDir)) {
      return preferredPath;
    }
    const wantedKey = this.normalizeMapAssetKey(mapName);
    const files = fs.readdirSync(textZonesDir).filter((name) => name.toLowerCase().endsWith(".text-zones.json"));
    for (const fileName of files) {
      const stem = fileName.replace(/\.text-zones\.json$/i, "");
      if (this.normalizeMapAssetKey(stem) === wantedKey) {
        return path.join(textZonesDir, fileName);
      }
    }
    return preferredPath;
  }
  ...
  getMapZones(mapId: string): ZonesPayload {
    const config = this.getMapAdminConfig(mapId);
    const filePath = this.resolveExistingZonesPath(this.resolveZonesPath(mapId), config.mapName);
  ...
  updateMapZones(mapId: string, payload: ZonesPayload): ZonesPayload {
    ...
    const zones = Array.isArray(payload.zones)
      ? payload.zones
          .map((zone, idx) => this.normalizeZonePolygon(zone, idx, imageWidth, imageHeight))
  ...
  private normalizeZonePolygon(
    zone: ZonesPayload["zones"][number],
    index: number,
    imageWidth?: number,
    imageHeight?: number
  ): ZonesPayload["zones"][number] {
    const allowedTypes = new Set(["forbidden", "transient", "trusted", "respawn"]);
    const zoneTypeRaw = String(zone?.type ?? "forbidden").toLowerCase();
    const zoneType = allowedTypes.has(zoneTypeRaw) ? zoneTypeRaw : "forbidden";
    ...
    const polygonRaw = Array.isArray(zone?.polygon) ? zone.polygon : [];
    const polygon = polygonRaw
      ...
      .map((point) => {
        ...
        if (safeWidth !== undefined && safeWidth > 0) x = Math.max(0, Math.min(x, safeWidth - 1));
        if (safeHeight !== undefined && safeHeight > 0) y = Math.max(0, Math.min(y, safeHeight - 1));
        return [Math.round(x), Math.round(y)];
      });
    return {
      id: String(zone?.id ?? `${zoneType}_${index + 1}`),
      type: zoneType as ZonesPayload["zones"][number]["type"],
      polygon,
      ...(zoneType === "transient"
        ? { max_dwell_sec: Math.max(0, Number(zone?.max_dwell_sec ?? 8)) }
        : {}),
    };
  }
  ...
  getMapTextZones(mapId: string): TextZonesPayload {
    const config = this.getMapAdminConfig(mapId);
    const filePath = this.resolveExistingTextZonesPath(this.resolveTextZonesPath(mapId), config.mapName);
    if (!fs.existsSync(filePath)) {
      return {
        map: config.mapName,
        image_size: { width: 0, height: 0 },
