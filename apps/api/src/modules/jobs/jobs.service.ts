...
interface CreateRunningAnalysisJobInput {
  command: string;
  video?: string;
  mapId?: string;
  matchId?: string;
...
export class JobsService {
  ...
  private async saveUpdatedJobPg(record: JobRecord): Promise<JobRecord> {
    await this.postgresPool.query(
      `UPDATE api_jobs
       SET status = $2,
           command = $3,
  ...
  private saveUpdatedJobFile(record: JobRecord): JobRecord {
    const store = this.loadStore();
    const idx = store.jobs.findIndex((entry) => entry.id === record.id);
    if (idx >= 0) {
  ...
  private async saveUpdatedJob(record: JobRecord): Promise<JobRecord> {
    if (this.jobsSourceMode !== "sqlite") {
      try {
        return await this.saveUpdatedJobPg(record);
      } catch (error) {
        if (this.jobsSourceMode === "postgres") throw error;
        // fallback to file store
      }
    }
    return this.saveUpdatedJobFile(record);
  }

  async createRunningAnalysisJob(input: CreateRunningAnalysisJobInput): Promise<JobRecord> {
  ...
  async updateJobProgress(jobId: string, patch: UpdateJobPatch): Promise<JobRecord> {
    const record = await this.getJob(jobId);
    const next: JobRecord = {
      ...record,
      status: record.status === "queued" ? "running" : record.status,
      currentAction: patch.currentAction ?? record.currentAction,
      lastHeartbeatAt: new Date().toISOString(),
      progressPercent:
        patch.progressPercent !== undefined
          ? Math.max(0, Math.min(100, Number(patch.progressPercent) || 0))
          : record.progressPercent,
      payload: patch.payload ? { ...record.payload, ...patch.payload } : record.payload,
      errors: patch.errors ? [...(record.errors ?? []), ...patch.errors] : record.errors,
    };
    return this.saveUpdatedJob(next);
  }

  async completeJob(jobId: string, patch: UpdateJobPatch = {}): Promise<JobRecord> {
    const record = await this.getJob(jobId);
    const finishedAt = new Date().toISOString();
    const started = record.startedAt ? Date.parse(record.startedAt) : Date.parse(record.queuedAt);
    const finished = Date.parse(finishedAt);
    const next: JobRecord = {
      ...record,
      status: "completed",
      currentAction: patch.currentAction ?? "completed",
      lastHeartbeatAt: finishedAt,
      progressPercent: patch.progressPercent ?? 100,
      finishedAt,
      durationMs: Number.isFinite(started) && Number.isFinite(finished) ? Math.max(0, finished - started) : record.durationMs,
      payload: patch.payload ? { ...record.payload, ...patch.payload } : record.payload,
  ...
  async failJob(jobId: string, errorMessage: string, patch: UpdateJobPatch = {}): Promise<JobRecord> {
    const record = await this.getJob(jobId);
    const finishedAt = new Date().toISOString();
    const started = record.startedAt ? Date.parse(record.startedAt) : Date.parse(record.queuedAt);
    const finished = Date.parse(finishedAt);
    const next: JobRecord = {
      ...record,
      status: "failed",
      currentAction: patch.currentAction ?? "failed",
      lastHeartbeatAt: finishedAt,
      progressPercent: patch.progressPercent ?? record.progressPercent,
      finishedAt,
      durationMs: Number.isFinite(started) && Number.isFinite(finished) ? Math.max(0, finished - started) : record.durationMs,
      payload: patch.payload ? { ...record.payload, ...patch.payload } : record.payload,
      errors: [...(record.errors ?? []), errorMessage, ...(patch.errors ?? [])],
    };
    return this.saveUpdatedJob(next);
  }

  async patchJobPayload(jobId: string, payloadPatch: Record<string, unknown>, currentAction?: string): Promise<JobRecord> {
    const record = await this.getJob(jobId);
    const next: JobRecord = {
      ...record,
      currentAction: currentAction ?? record.currentAction,
      lastHeartbeatAt: new Date().toISOString(),
      payload: { ...record.payload, ...payloadPatch },
    };
    return this.saveUpdatedJob(next);
  }

  async controlJob(jobId: string, action: "pause" | "resume" | "cancel"): Promise<JobRecord> {
    const job = await this.getJob(jobId);
    const controlPath = typeof job.payload.controlPath === "string" ? job.payload.controlPath : "";
    const control = { action, updatedAt: new Date().toISOString() };
    if (controlPath) {
      fs.mkdirSync(path.dirname(controlPath), { recursive: true });
      fs.writeFileSync(controlPath, JSON.stringify(control), "utf-8");
    }
    if (action === "cancel") {
      const pid = Number(job.payload.pid);
      ...
      if (Number.isInteger(pid) && pid > 0) {
        ...
        killError = "PID is not available for this job";
      }
      return this.failJob(jobId, "Cancelled by user", {
    ...
    return this.patchJobPayload(jobId, { control }, action === "resume" ? "running" : action);
  }
  ...
  async listJobs(filters: {
    ...
    if (this.jobsSourceMode !== "sqlite") {
      try {
        ...
        if (this.jobsSourceMode === "postgres") return pgResult;
  ...
  async getJob(jobId: string): Promise<JobRecord> {
    if (this.jobsSourceMode !== "sqlite") {
      try {
        ...
        if (this.jobsSourceMode === "postgres") {
          throw new NotFoundException(`Job not found in postgres: ${jobId}`);
        }
      } catch (error) {
        if (this.jobsSourceMode === "postgres") throw error;
        // fallback to file store
